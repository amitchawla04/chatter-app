...
- Name: new-features.spec.ts >> Village threads — auth-gated full create flow >> /v/new shows topic picker + name input + invite textarea
- Location: e2e/new-features.spec.ts:107:7
...
Locator: getByText(/new thread/i).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText(/new thread/i).first()
...
> 117 |     await expect(page.getByText(/new thread/i).first()).toBeVisible();
  ...
  206 | });
  207 | 
  208 | test.describe("i18n locale switching", () => {
  209 |   test("POST /api/locale with valid locale sets cookie + 200", async ({ request }) => {
  210 |     const res = await request.post(`${BASE}/api/locale`, {
  211 |       data: { locale: "es" },
  212 |       headers: { "Content-Type": "application/json" },
  213 |     });
  214 |     expect(res.status()).toBe(200);
  215 |     const json = await res.json();
  216 |     expect(json.ok).toBe(true);
  217 |     expect(json.locale).toBe("es");
```